...
html.summary.lm <- function (x, digits = max(3L, getOption("digits") - 3L), ...) {
  ...
  tag.table <- function(x, ...) tag.print(HTML(kable(as.data.frame(x), format="html", table.attr = "class='table'", ...)))
  ...
  if (length(x$aliased) == 0L) {
    ...
    tag.print(tags$div(style="margin-top:-10px", 
      "Signif. codes:", 
      tags$pre(attr(Signif,"legend")), tags$br()
    ))
  }
...
The following datasets are available for analysis using the web-based tool. You can specify a custom dataset, by choosing "Custom..." and uploading your data file (in .CSV or .XLSX format).
...
```{r context="render"}
formattableOutput("correlationTable")
```

```{r context="server"}
output$correlationTable <- renderFormattable({
  shiny::validate(
